IF OBJECT_ID('data_mart.machine_learning', 'V') IS NOT NULL
    DROP VIEW data_mart.machine_learning;
GO

CREATE VIEW data_mart.machine_learning AS 
SELECT 
...
FROM data_mart.fact_customer_metrics f
LEFT JOIN data_mart.dim_demographics dem
    ON f.customer_id = dem.customer_id 
LEFT JOIN data_mart.dim_account acc
    ON f.customer_id = acc.customer_id
LEFT JOIN data_mart.dim_status sta
    ON f.customer_id = sta.customer_id
LEFT JOIN data_mart.dim_services ser
